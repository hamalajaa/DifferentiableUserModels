...
function MenuAssistant()
	MenuAssistant(nothing, nothing, nothing, nothing, nothing, false, 0., nothing, nothing, nothing, nothing, nothing, nothing, nothing, [], false, 0, true, false, true, [])
end
...
function reset(mdp::MenuAssistant)
	...
	if mdp.n_traj % 20 == 0
		...
		action_map1 = collect(POMDPModelTools.actions(env1.m))
		action_indices1 = Dict(a=>i for (i, a) in enumerate(action_map1))

		action_map2 = collect(POMDPModelTools.actions(env2.m))
