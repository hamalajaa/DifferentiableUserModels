...
args1 = Dict("gen" => "menu_search", "n_traj" => 0, "params" => false, "p_bias" => 0.0, "perm" => false)
...
for e in 0:n_epochs
    ...
    @printf("Epoch:  %-6d\n", e)
    if (e % 100) == 0
       	bson("models/ex2/maml_v2/"*string(e)*".bson", model=m)
    end
...
bson("results/ex2/maml_v2.bson", means_t=means_t, cis_t=cis_t, 
